...
train_path = 'data/NPC_3/train_set'
model_start = 'models/model_original'
model_name = 'models/model_NPC_3_new'
steps_per_epoch = 64
max_epochs = 2000
...
callbacks = [EarlyStopping(monitor='val_loss', patience=early_stopping_patience),
            ModelCheckpoint(filepath='models/' + model_name + '/best_weights.h5', monitor='val_loss', save_best_only=True)]
...
plt.savefig(model_name + '/training_accuracy.png')
plt.clf()
...
plt.xlabel('epoch')
plt.legend(['train', 'val'], loc='upper left')
plt.savefig(model_name + '/training_loss.png')
